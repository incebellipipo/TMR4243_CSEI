...
class JoystickForce(rclpy.node.Node):
    ...
    def timer_callback(self):
        ...
        self.get_logger().info(f"Parameter task: {self.current_task.value}", throttle_duration_sec=1.0)
    ...
    def joy_callback(self, msg):
        ...
        if len(result) != 5:
            self.get_logger().warn(f"Result has length of {len(result)} but it should be 5", throttle_duration_sec=1.0)
