...
def isTopic(cursor, topic_name, print_out=False):
    ...
    if print_out:
        if boolIsTopic:
            ...
            print('\nTopic named', topicFound[1],
                  ' exists at id ', topicFound[0], '\n')
        else:
...
if __name__ == "__main__":
    ...
    type_map = {topic_names[i]: topic_types[i]
    ...
    for topic_name in topic_names:
        ...
        csv_file = os.path.join(
            csv_directory, topic_name.replace('/', '_') + ".csv")
        ...
        with open(csv_file, 'w', newline='') as csvfile:
            ...
            if topic_name == "/CSEI/observer/state":
                csv_writer.writerow(["Timestamp", "eta.x", "eta.y", "eta.z",
                                    "nu.u", "nu.v", "nu.w", "bias.x", "bias.y", "bias.z"])
                for timestamp, message in zip(t, msgs):
                    ...
                    csv_writer.writerow([timestamp, deserialized_msg.eta[0], deserialized_msg.eta[1], deserialized_msg.eta[2], deserialized_msg.nu[0],
                                        deserialized_msg.nu[1], deserialized_msg.nu[2], deserialized_msg.bias[0], deserialized_msg.bias[1], deserialized_msg.bias[2]])
            elif topic_name == "/CSEI/control/u_cmd":
                csv_writer.writerow(
                    ["Timestamp", "u_cmd.1", "u_cmd.2", "u_cmd.3", "u_cmd.4", "u_cmd.5"])
                for timestamp, message in zip(t, msgs):
                    deserialized_msg = deserialize_message(message, msg_type)
                    seconds = timestamp // 10**9
                    nanoseconds = timestamp % 10**9
                    timestamp = f"{seconds}.{nanoseconds}"
                    csv_writer.writerow([timestamp, deserialized_msg.data[0], deserialized_msg.data[1],
                                        deserialized_msg.data[2], deserialized_msg.data[3], deserialized_msg.data[4]])
                ...
                for timestamp, message in zip(t, msgs):
                    ...
                    timestamp = f"{seconds}.{nanoseconds}"
                    csv_writer.writerow(
                        [timestamp, deserialized_msg.data[0], deserialized_msg.data[1], deserialized_msg.data[2]])
                ...
                csv_writer.writerow(
                    ["Timestamp", "force.x", "force.y", "force.z", "torque.x", "torque.y", "torque.z"])
                for timestamp, message in zip(t, msgs):
                    deserialized_msg = deserialize_message(message, msg_type)
                    seconds = timestamp // 10**9
                    nanoseconds = timestamp % 10**9
                    timestamp = f"{seconds}.{nanoseconds}"
                    csv_writer.writerow([timestamp, deserialized_msg.force.x, deserialized_msg.force.y, deserialized_msg.force.z,
                                        deserialized_msg.torque.x, deserialized_msg.torque.y, deserialized_msg.torque.z])
